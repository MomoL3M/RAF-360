...
import { ECHEANCES, statutColor } from "../data/demo.js";
...
export default function Calendar() {
  return (
    <>
      <SectionTitle title="Calendrier des obligations" sub="Trié de la date la plus proche à la plus lointaine — chaque règle est datée et sourcée" />
      <AiInsight
        sev={C.navy2}
        kind="Anticipation"
          ...
          return (
            <div className="list-row" key={i} style={od ? { background: C.redl } : undefined}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 48, height: 48, borderRadius: 10, background: col + "18", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                ...
                <div>
                  <div style={{ fontSize: 15, color: C.ink, fontWeight: 600 }}>
                    {e.t}
                    {e.montant && (
                      <span className="chip" style={{ marginLeft: 6, color: e.mt === "réel" ? C.slate : C.gold, borderColor: e.mt === "réel" ? C.line : C.gold + "55" }}>
                        {e.montant} · {e.mt}
                      </span>
                    )}
                  </div>
                  <div style={{ fontSize: 12, color: C.slate }}>
                    Priorité {e.prio.toLowerCase()}{od && <> · <b style={{ color: C.red }}>en retard</b></>}
                  </div>
                </div>
              </div>
