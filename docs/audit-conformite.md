...
- **Page Tarifs (✅ 2026-07-28, commit `367106a`)** : `/tarifs` iso-graphisme (hero, 4 cartes, tableau comparatif, FAQ, CTA) avec les **4 offres figées R1** (Starter 39 / Pilot TPE 129 / RAF PME 349 / RAF ETI 899 € — la baseline n'en présentait que 3, fictives). Garde-fous appliqués (« facturation électronique », hébergement « Union européenne », CTA diagnostic). **GEO** : réponse autoportante + **FAQPage JSON-LD** (validé). **A11y** : libellés lecteur d'écran sur le comparatif. CSS ajouté : `.price-grid--4`, `.compare-table`, `.visually-hidden`. Recette : HTTP 200, PHPStan niv.6 0, CS-Fixer 0, JSON-LD parse OK.
- **Page Produit (✅ 2026-07-28, commit `cf32bae`)** : `/produit` iso-graphisme — hero + 6 modules alternés (copilote, trésorerie, échéances, coffre, e-facture, data room) + CTA. Garde-fous appliqués.
- **Ajustements accueil (✅ 2026-07-28, commit `52985f1`, retours PM)** : titres sans point ; accroche englobante « Le poste de pilotage de votre entreprise » ; courbe de trésorerie **ascendante + 75 000 € en vert** ; **compteurs animés au scroll** (Stimulus `countup`) ; 4 étapes affichées en permanence ; « Sérieux réglementaire » → « conformité réglementaire, par conception » ; **€ insécable** avec le montant ; garde-fou no-JS (`.js .reveal`). Logos officiels **refusés** (usage commercial non autorisé + garde-fou label/partenariat) → sources citées en texte.
- **Site public COMPLÉTÉ (✅ 2026-07-28, commit `a76dbed`)** : Solutions, À propos, Blog (état vide honnête), **Contact + Diagnostic (formulaires réels** : DTO+Validator, CSRF, honeypot, rate limit, PRG, flash — §13.1/§16), pages **légales** (mentions/confidentialité/RGPD, vraies données éditeur + `TODO-PM` visibles). Réconciliation **hébergement « France »→« UE »** partout ; retrait de « Groupe ARCAN » (non validé). Template « en construction » supprimé. **11 routes GET 200, POST formulaires OK, PHPStan 0, CS-Fixer 0.**
  - ⚠ **`TODO-PM` restants (bloquants avant mise en ligne)** : email de contact, directeur de publication, **hébergeur (UE)**, DPO, durées de conservation, **validation juridique** des pages légales, **témoignages/chiffres réels**.
- **Reste (lots backend/infra, hors site public)** : `/app` (interface réelle du SaaS) ; **moteur de diagnostic SIREN** (API INSEE/SIRENE + analyse — aujourd'hui le formulaire capture le lead) ; **bandeau cookies/consentement** ; SEO/GEO plumbing (sitemap, robots.txt + robots IA, `llms.txt`, JSON-LD Organization, canonical) ; NelmioSecurityBundle (CSP/HSTS) ; LiipImagine ; observabilité (`/health`, Sentry, emails transactionnels SPF/DKIM/DMARC) ; **recette visuelle 4 largeurs vs baseline** (R5) → conformité §25 → recette R5.
