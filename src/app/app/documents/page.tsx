...
import { DOC_TREE } from "@/data/demo";
import { SectionTitle, Badge } from "@/components/ui";
import AiInsight from "@/components/AiInsight";

export default function Documents() {
  const [open, setOpen] = useState<Record<string, boolean>>({ corp: true, biz: false, rh: false });
  ...
  return (
    <>
      <SectionTitle
        ...
        sub="Dépôt, OCR et classement automatique par arborescence — score de confiance par pièce"
      />
      <AiInsight
        sev={C.gold}
      ...
      >
        ...
        <div style={{ fontSize: 13, color: C.slate, marginTop: 4 }}>
          ou par email, mobile, scanner · PDF, images, XML acceptés — classement automatique
        </div>
        <div style={{ marginTop: 12 }}>
          <button className="btn btn-primary btn-sm">Parcourir</button>
        </div>
      </div>

      <div className="card" style={{ padding: 0 }}>
        {Object.keys(DOC_TREE).map((key) => {
          const f = DOC_TREE[key],
            isOpen = open[key];
          return (
            <div key={key}>
                ...
                <>
                  ...
                  {f.files.map((d, i) => (
                    <div className="tree-child" key={i}>
                      ...
                      <span>
                        <Badge color={C.navy2}>{d.type}</Badge>
                      </span>
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 13, color: C.slate, marginTop: 3 }}>
            Estimez un coût employeur, un net à payer ou une prime — pré-contrôle avant transmission au gestionnaire.
          </div>
        </div>
