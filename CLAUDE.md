...
- **Projet démarré AVANT le standard — mise en conformité selon `docs/audit-conformite.md` (protocole 0-R). Phase en cours : R4-B, lot 2 (règles métier & entités).** Faits : R0 (git + tag + état des lieux), R2 (inventaire de transfert `docs/inventaire-transfert.md` ✅ ; audit d'écarts par section en cours), R4-B lot 1 (socle Symfony 7.4 + Docker FrankenPHP/PostgreSQL ✅). Reste R1 : fiche projet `docs/fiche-projet.md` à remplir par le chef de projet. _(mettre à jour au fil de l'avancement)_
...
- **Installé (lot 1)** : Twig, Symfony UX (Stimulus/Turbo), AssetMapper, Doctrine ORM, Validator, Docker (FrankenPHP + PostgreSQL 16). **Reste à installer** (lots suivants) : SecurityBundle, NelmioSecurityBundle, sitemap, LiipImagineBundle, CI (GitHub Actions), `llms.txt`/`robots.txt`. _(efface au fur et à mesure)_
