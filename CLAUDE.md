...
- **Phase en cours : R5 (recette & bascule). R4-B est TERMINÉE.** Recette §25 déroulée avec preuve par item → **`docs/recette-r5.md`** : ⛔ mise en ligne **BLOQUÉE** (9 bloquants, dont 5 côté chef de projet : témoignages/chiffres, email + directeur de publication + hébergeur, CGV/CGU, domaine/certificat, arbitrage CG-01). Restent techniquement : export/suppression RGPD, sauvegardes + PRA + rollback, plan de mesure/analytics, test de fumée + Renovate. **Non vérifiables sans préproduction** : Lighthouse/CWV, accessibilité clavier + lecteur d'écran, recette visuelle 4 largeurs. **⚠️ Gate R0 toujours ouvert : aucun dépôt distant privé.**
