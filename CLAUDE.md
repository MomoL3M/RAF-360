...
> - Projet : **RAF360** (copilote de pilotage financier/comptable/fiscal/social/juridique pour TPE-PME, tous secteurs — éditeur **Lindbergh Formation SAS**, SIRET 817 946 114 00029)
...
- **⚠️ Ce dépôt est GELÉ — il ne sert plus que de baseline (chemin B).** Le travail vit dans **`../raf360-symfony`**, dont le `CLAUDE.md` porte désormais le suivi à jour. Sauvegardé hors machine : branche `baseline-nextjs-gelee` + tag `avant-mise-en-conformite` sur `github.com/MomoL3M/RAF-360`.
- **Phase en cours : R5 (recette & bascule).** R0/R1/R2/R3 ✅, R4-B ✅ (site public + 8 écrans `/app` sur vraies données + auth Argon2id + MFA admin), recette §25 avec preuve par item et 4 bloquants techniques soldés (RGPD exerçable, sauvegardes testées, plan de mesure, test de fumée). ⛔ Mise en ligne bloquée par **5 points côté chef de projet** : témoignages/chiffres réels, email de contact + directeur de la publication + hébergeur UE, CGV/CGU, domaine + certificat, arbitrage CG-01. Détail : `../raf360-symfony/docs/recette-r5.md`.
...
- **Installé** : Twig, Symfony UX (Stimulus/Turbo), AssetMapper, Doctrine ORM, Validator, SecurityBundle, RateLimiter, Docker (FrankenPHP + PostgreSQL 16), CI GitHub Actions, PHPStan + PHP-CS-Fixer, polices auto-hébergées. **Reste à installer** : NelmioSecurityBundle, sitemap, LiipImagineBundle, `llms.txt`/`robots.txt`.
- **Dérogation §2.3 (taille de fichiers)** : le design system `assets/styles/marketing.css` (~640 l.) et `templates/marketing/accueil.html.twig` (~250 l.) dépassent les plafonds 300/200 lignes — cohésion de la cascade et des sections privilégiée. _(dérogation assumée ; refactor en composants possible)_
